...
namespace ThGameMgr.Ex.Score.Th15
{
    ...
    {
        private static readonly string[] _th15PlayersList = GamePlayers.GetGamePlayers(GameIdIndex.Th15).Split(',');
        ...
        {
            string? gamePath = GameFile.GetGameFilePath(GameIdIndex.Th15);
            string? scorePath = ScoreFile.GetScoreFilePath(GameIdIndex.Th15);

            if (File.Exists(gamePath) && File.Exists(scorePath))
            {
                MemoryStream decodedData = new();
                bool decodeResult = ScoreDecoder.Decode(GameIdIndex.Th15, scorePath, decodedData);
                if (decodeResult)
                {
                    ...
                    using (decodedData)
                    {
                        ...
                        for (int p = 1; p < 120; p++)
                        {
                            ...
                            foreach (string player in GamePlayers.GetGamePlayers(GameIdIndex.Th15).Split(','))
                            {
            ...
            int cardId, byte[] data)
        {
            ...
            string[] players = GamePlayers.GetGamePlayers(GameIdIndex.Th15).Split(',');
